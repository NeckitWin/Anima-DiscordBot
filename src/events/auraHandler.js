...
export default {
    ...
    async execute(message) {
        try {
            if (!(message.content === '-aura' || message.content === '+aura'))
                return;
            if (!await commandLog("auraButtons", message, 0))
                return;
            const lang = await getLang(message);
            const local = lang.aura;
            const replyUser = message.mentions.repliedUser;
            if (replyUser === null || replyUser === undefined)
                return message.reply({
                    content: lang.error.mustreply,
                    ephemeral: true
                });
            if (replyUser.id === message.author.id)
                return message.reply(local.cantyourself);
            if (replyUser.bot)
                return message.reply(local.cantbot);
            if (!await setCooldown("aura", message, 60))
                return;
