    ...
    async execute(interaction) {
        try {
            ...
            if (interaction.customId !== `postModal`)
                return;
            if (!await commandLog("postHandle", interaction, 1))
                return;
            ...
            if (!text && !description && !image)
                errorMessage = local.errorContent;
            if (color && !/^#[0-9A-F]{6}$/i.test(color))
                errorMessage = local.errorColor;
            if (authorID && !mebmer)
                errorMessage = local.errorAuthor;
            if (image && !/^https?:\/\/.+\..+/i.test(image))
                errorMessage = local.errorImage;
            if (errorMessage) {
