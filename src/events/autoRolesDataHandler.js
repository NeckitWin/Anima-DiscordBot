import { Events, RoleSelectMenuBuilder, ActionRowBuilder, EmbedBuilder, StringSelectMenuBuilder, ButtonBuilder } from 'discord.js';
...
export default {
    ...
    async execute(interaction) {
        try {
            ...
            if (!await commandLog("autoRolesDataHandler", interaction, 1))
                return;
            ...
            if (customId === 'autoRolesMenu' || customId === 'returnAutoRoles' || customId === 'clearAutoRoles') {
                ...
                for (const role of getRoles) {
                    if (typeof role === "string")
                        await removeAutoRole(guild.id, role);
                }
