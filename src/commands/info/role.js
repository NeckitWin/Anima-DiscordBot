    ...
    async execute(interaction) {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: `🆔 ${local.roleid}`, value: "```fix\n" + role.id + "```", inline: true }, { name: `🌈 ${local.color}`, value: "```" + "#" + role.color.toString(16) + "```", inline: false }, { name: `📅 ${local.date}`, value: "```" + formatDate(role.createdAt) + "```", inline: true }, { name: `👥 ${local.owners}`, value: "```" + role.members.size + "```", inline: true }, {
                name: `👑 ${local.position}`,
